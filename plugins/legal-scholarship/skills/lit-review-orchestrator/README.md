...
> **Undermind — terms-of-service note.** The Undermind stage (Stage 1) automates a
> logged-in browser session against Undermind, a paid deep-search service that
> publishes no public API, using your own paid account and stored credentials.
> Automated access of this kind may fall outside Undermind's terms of service, so
> you run this stage at your own risk and under your own account. It is optional:
> skip it with `--skip undermind`, by unchecking **Undermind** in the GUI, or with
> `--quick`, and the pipeline still runs on its remaining channels — Gemini Deep
> Research, SearchAPI Google Scholar, and the keyless web and free-index searches —
> with deduplication, verification, and screening unchanged. The opt-in Scholar
> Labs stage (Stage 2) carries the same consideration, since it likewise automates
> a logged-in Google Scholar session; it is off by default.
...
- **Undermind (Stage 1)** drives the Undermind.ai Classic deep search through Playwright,
  ...
  pipeline continues. This stage automates a logged-in session against a paid
  service under your own account and is optional — see the terms-of-service note
  under [Requirements](#requirements) and skip it with `--skip undermind` if you
  prefer.
