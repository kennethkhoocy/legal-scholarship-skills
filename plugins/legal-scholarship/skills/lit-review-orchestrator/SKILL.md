...
against Undermind, a paid deep-search service that publishes no public API, using
your own paid account and the credentials stored in `~/.lit-review-pipeline.env`.
Automated access of this kind may fall outside Undermind's terms of service, so you
run the stage at your own risk and under your own account. It is optional — the
orchestrator runs without it. Skip it with `--skip undermind`, by unchecking
**Undermind** in the GUI, or with `--quick`, and the remaining channels (Gemini
Deep Research, SearchAPI Google Scholar, and the keyless web and free-index
searches) carry the run, with dedup, verification, and screening unchanged. The
opt-in Scholar Labs stage (Stage 2) automates a logged-in Google Scholar session
in the same way and is likewise off by default.
